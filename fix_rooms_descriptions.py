...
while i < len(lines):
    ...
    if '{{ asp.description }}' in line and i + 2 < len(lines):
        ...
        if '</div>' in lines[i+1] and '</div>' in lines[i+2] and i+3 < len(lines) and '{% endif %}' in lines[i+3]:
            ...
            new_lines.append(indent + '<small class="text-muted fst-italic">Tidak ada deskripsi untuk aspek ini.</small>\n')
