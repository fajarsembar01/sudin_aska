...
def telegram_notifications() -> Response:
    ...
    admin_ids = {
        str(user.get("id")) for user in admin_users if user.get("id") is not None
    }
    ...
    if request.method == "POST":
        ...
        if action == "save_token":
            raw_token = request.form.get("bot_token") or ""
            upsert_telegram_notification_settings(raw_token, actor.get("id"))
            ...
            if entries_map:
                ...
                flash(f"{saved} admin Telegram disimpan.", "success")
            else:
                if not errors:
            ...
            if result.get("skipped") == "token_missing":
                ...
                if sent == 0 and group_sent == 0:
                    flash(
                        "Tes notifikasi belum terkirim. Pastikan admin sudah chat bot atau grup terdaftar.",
                        "warning",
                    )
                else:
                    flash(
                        f"Tes notifikasi terkirim ke {sent} admin dan {group_sent} grup.",
                        "success",
                    )
                if missing:
                    flash(f"{len(missing)} admin belum pernah chat bot.", "warning")

    settings = fetch_telegram_notification_settings()
