...
def seed_portal_data():
    ...
    with get_cursor(commit=True) as cur:
        ...
        cur.execute(
            "SELECT id FROM portal_kelurahan WHERE name = 'Semper Barat' LIMIT 1"
        )
        ...
        cur.execute(
            "SELECT id FROM portal_rooms WHERE active = TRUE ORDER BY sort_order"
        )
