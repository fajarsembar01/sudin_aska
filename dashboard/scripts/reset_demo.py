...
def reset_demo_school():
    ...
    with get_cursor(commit=True) as cur:
        ...
        if not school:
            print(f"School {NPSN} not found in portal_schools. Registration might fail if it relies on seeded data.")
        ...
        cur.execute("SELECT id, email, full_name FROM dashboard_users WHERE school_id = %s", (school_id,))
        users = cur.fetchall()
        ...
        if users:
            ...
            # Delete them
            cur.execute("DELETE FROM dashboard_users WHERE school_id = %s", (school_id,))
            print("Users deleted.")
        else:
