...
export default function EvaluasiSPMB() {
  ...
  const lastSpokenCallKeyRef = useRef<string | null>(null)
  const queueAudioEnabledRef = useRef(false)
  ...
  const loadQueueCounter = async (silent = false) => {
    ...
    try {
      ...
      if (nextCall?.id) {
        const callKey = `${nextCall.id}:${nextCall.calledAt || nextCall.updatedAt || ''}`
        if (lastSpokenCallKeyRef.current === null) {
          lastSpokenCallKeyRef.current = callKey
        } else if (lastSpokenCallKeyRef.current !== callKey) {
          lastSpokenCallKeyRef.current = callKey
          speakQueueCall(nextCall)
        }
      }
    } catch (error) {
      if (!silent) {
  ...
  const speakQueueCall = (call: QueueCall) => {
    if (typeof window === 'undefined' || !('speechSynthesis' in window)) {
      setQueueAudioMessage('Browser tidak mendukung suara otomatis')
      return
    }
    const text = call.announcement || `Nomor antrian ${call.queueNumber}, silakan menuju meja nomor ${call.tableNumber}.`
    ...
    utterance.pitch = 1
    utterance.onstart = () => {
      queueAudioEnabledRef.current = true
      setQueueAudioEnabled(true)
      setQueueAudioMessage('Memutar panggilan')
    }
    utterance.onend = () => setQueueAudioMessage('Suara aktif')
    utterance.onerror = () => setQueueAudioMessage('Klik Aktifkan Suara')
    window.speechSynthesis.speak(utterance)
  }

  const enableQueueAudio = () => {
    if (typeof window === 'undefined' || !('speechSynthesis' in window)) {
      setQueueAudioMessage('Browser tidak mendukung suara otomatis')
      return
    }
    ...
    utterance.onstart = () => {
      queueAudioEnabledRef.current = true
      setQueueAudioEnabled(true)
      setQueueAudioMessage('Suara aktif')
    }
    utterance.onend = () => setQueueAudioMessage('Suara aktif')
    utterance.onerror = () => {
      queueAudioEnabledRef.current = false
      setQueueAudioEnabled(false)
      setQueueAudioMessage('Suara diblokir browser')
    }
    window.speechSynthesis.speak(utterance)
  }
        ...
        <div className={`grid min-h-0 flex-1 gap-3 lg:items-stretch ${hasSpmbSummaryData ? 'lg:grid-cols-[5fr_3fr_3fr]' : 'lg:grid-cols-[5fr_3fr]'}`}>
          ...
          <section className="flex min-h-0 flex-col rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:h-full">
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <div className="min-w-0">
                <h2 className="truncate text-sm font-extrabold">Riwayat Evaluasi</h2>
                <div className="mt-1 flex flex-wrap gap-1 text-[10px] font-extrabold text-slate-600">
                  <span className="rounded-full bg-slate-100 px-1.5 py-0.5">Hari ini {evaluationSummary.today}</span>
                  <span className="rounded-full bg-slate-100 px-1.5 py-0.5">Total {evaluationSummary.total}</span>
                  <span
                    className={`rounded-full px-1.5 py-0.5 ${queueAudioEnabled ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}
                    title={queueAudioMessage}
                  >
                    {queueAudioEnabled ? 'Suara aktif' : 'Suara off'}
                  </span>
                </div>
              </div>
              <div className="flex shrink-0 flex-wrap justify-end gap-1.5">
                ...
                >
                  {queueAudioEnabled ? 'Suara Aktif' : 'Aktifkan Suara'}
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="rounded-md border border-slate-200 px-2.5 py-1 text-[11px] font-bold text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
                >
