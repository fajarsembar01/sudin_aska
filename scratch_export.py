...
cur = conn.cursor()
cur.execute('SELECT id, tanggal_edit FROM hospitality_guestbook_reviews WHERE tanggal_edit IS NOT NULL')
rows = cur.fetchall()
...
for r in rows:
    ...
    updates.append(f"UPDATE hospitality_guestbook_reviews SET tanggal_edit = '{dt_str}' WHERE id = {r[0]};")

with open('server_update_tanggal_edit.sql', 'w') as f:
    f.write('-- Script SQL untuk menambahkan dan mengupdate kolom tanggal_edit di SERVER\n\n')
