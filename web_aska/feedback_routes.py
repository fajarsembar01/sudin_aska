...
from db import delete_feedback, get_chat_history, get_feedback_status, save_feedback
...
def submit_feedback():
    ...
    if not feedback_type:
        return jsonify({"success": False, "error": "feedback_type is required"}), 400
    ...
    try:
            ...
            jsonify(
                {
                    ...
                    "feedback": {
                        ...
                        "created_at": (
                            ...
                            if hasattr(feedback["created_at"], "isoformat")
        ...
        return jsonify({"success": False, "error": "Invalid chat_log_id"}), 400
...
def remove_feedback(chat_log_id: int):
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Feedback removed"}), 200
...
def get_feedback_statuses():
    ...
    if not chat_log_ids_str:
        return (
            jsonify({"success": False, "error": "chat_log_ids parameter is required"}),
            400,
        )
    ...
    try:
        chat_log_ids = [
            int(id_str.strip())
            for id_str in chat_log_ids_str.split(",")
            if id_str.strip()
        ]
    except ValueError:
        return jsonify({"success": False, "error": "Invalid chat_log_ids format"}), 400
            ...
            if feedback:
                formatted_feedbacks[str(chat_log_id)] = {
                    ...
                    "updated_at": (
                        ...
                        if hasattr(feedback["updated_at"], "isoformat")
                        else str(feedback["updated_at"])
                    ),
                }
            else:
                formatted_feedbacks[str(chat_log_id)] = None

        return jsonify({"success": True, "feedbacks": formatted_feedbacks}), 200
