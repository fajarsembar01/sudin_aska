...
def migrate():
    ...
    with get_cursor(commit=True) as cur:
        ...
        try:
            cur.execute(
                "ALTER TABLE portal_assessments ADD COLUMN IF NOT EXISTS period_id INTEGER REFERENCES portal_assessment_periods(id)"
            )
            cur.execute(
        ...
        if not row:
            ...
            cur.execute(
                "SELECT id FROM portal_assessment_periods WHERE name = 'Semester 2 2025'"
            )
            row = cur.fetchone()
