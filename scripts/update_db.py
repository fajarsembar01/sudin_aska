...
def update_web_users_table():
    ...
    try:
        ...
        with db.conn.cursor() as cur:
            ...
            cur.execute(
                "ALTER TABLE web_users ADD COLUMN IF NOT EXISTS photo_url TEXT;"
            )

            print("   - Menambahkan kolom 'last_login'...")
            cur.execute(
                "ALTER TABLE web_users ADD COLUMN IF NOT EXISTS last_login TIMESTAMPTZ;"
            )

            print(
                "   - Mengisi nilai default untuk 'last_login' pada baris yang ada..."
            )
            cur.execute(
                "UPDATE web_users SET last_login = NOW() WHERE last_login IS NULL;"
            )

            print(
                "   - Mengatur 'last_login' agar NOT NULL dan memiliki nilai default..."
            )
            cur.execute("ALTER TABLE web_users ALTER COLUMN last_login SET NOT NULL;")
            cur.execute(
                "ALTER TABLE web_users ALTER COLUMN last_login SET DEFAULT NOW();"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "Satu atau lebih langkah pembaruan database gagal. Silakan periksa log error di atas.",
            file=sys.stderr,
        )
