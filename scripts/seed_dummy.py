...
def generate_dummy():
    with app.app_context():
        with get_cursor(commit=True) as cur:
            ...
            cur.execute(
                "SELECT id FROM portal_assessment_periods WHERE is_active = TRUE LIMIT 1"
            )
            period = cur.fetchone()
            if not period:
                cur.execute("""
            ...
            if not master_room:
                cur.execute(
                    "INSERT INTO portal_rooms (name, category) VALUES ('Ruang Kelas', 'Utama') RETURNING id"
                )
            ...
            for s in schools:
                ...
                status = random.choice(
                    ["draft", "submitted", "verified", "verified", "submitted"]
                )
                score = round(random.uniform(1.5, 3.0), 2)

                # Check exist
                cur.execute(
                    "SELECT id FROM portal_assessments WHERE school_id = %s AND period_id = %s",
                    (s["id"], period_id),
                )
                ...
                if status != "draft":
                    ...
                    cur.execute(
                        "SELECT id FROM portal_school_rooms WHERE school_id = %s AND room_id = %s",
                        (s["id"], master_room_id),
                    )
                    s_room = cur.fetchone()
                    if s_room:
                        ...
                        cur.execute(
                            "INSERT INTO portal_school_rooms (school_id, room_id, quantity) VALUES (%s, %s, 1) RETURNING id",
                            (s["id"], master_room_id),
                        )
