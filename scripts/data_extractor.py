...
def parse_excel(path):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        school = {
            ...
            "NPSN": (
                str(row[idx_npsn]).strip() if idx_npsn != -1 and row[idx_npsn] else "-"
            ),
            "Jenjang": (
                str(row[idx_jenjang]).strip()
                if idx_jenjang != -1 and row[idx_jenjang]
                else "-"
            ),
            "Status": (
                str(row[idx_status]).strip()
                if idx_status != -1 and row[idx_status]
                else "-"
            ),
            "Alamat": (
                str(row[idx_alamat]).strip()
                if idx_alamat != -1 and row[idx_alamat]
                else "-"
            ),
            "Kecamatan": (
                str(row[idx_kec]).strip()
                if idx_kec != -1 and row[idx_kec]
                else "Unknown"
            ),
            "Kelurahan": (
                str(row[idx_kel]).strip()
                if idx_kel != -1 and row[idx_kel]
                else "Unknown"
            ),
        }
...
def generate_markdown(header, structure):
    ...
    for kec in sorted_kec:
        ...
        for kel in sorted_kel:
            ...
            schools = sorted(
                structure[kec][kel], key=lambda x: x.get("Nama Sekolah", "")
            )
